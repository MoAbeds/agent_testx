      ...
      <div className="relative z-10 w-full max-w-md">
        ...
        <div className="relative bg-gray-900/60 backdrop-blur-xl border border-gray-800/50 rounded-2xl p-8 shadow-2xl">
          <div className="text-center mb-6">
            <span className="inline-block px-3 py-1 text-xs font-semibold text-terminal bg-terminal/10 border border-terminal/20 rounded-full mb-4">
              14-DAY FREE TRIAL
            </span>
          ...
          <div className="grid grid-cols-2 gap-2 mb-6">
            {trialFeatures.map((feature) => (
              <div key={feature} className="flex items-center gap-2 text-sm text-gray-300">
                <Check size={14} className="text-terminal flex-shrink-0" />
                <span>{feature}</span>
              </div>
            ))}
          </div>
