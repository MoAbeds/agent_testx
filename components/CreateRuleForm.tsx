...
export default function CreateRuleForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!siteId) {
      setNotification({ message: "Please add a site in the Overview page first.", type: "info" });
      return;
    }
    setLoading(true);
    ...
    try {
      ...
      setNotification({ message: "Manual rule deployed successfully!", type: "success" });
    } catch (error) {
      console.error(error);
      setNotification({ message: "Failed to create rule.", type: "error" });
    } finally {
      setLoading(false);
    }
  };
