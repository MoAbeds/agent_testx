...
export default function TerminalFeed() {
  ...
  const fetchLogs = async () => {
    try {
      ...
      if (Array.isArray(data)) {
        // Reverse so newest is at the bottom in the list logic, 
        // or just keep them sorted by date. 
        // Our API returns newest first. Let's flip for terminal display (oldest at top).
  ...
  const formatLog = (log: AgentLog) => {
    ...
    const isSuccess = log.type === 'SUCCESS' || log.type === 'REWRITE_META';
  ...
  return (
    <div className="bg-black border border-gray-800 rounded-lg overflow-hidden font-mono text-sm shadow-2xl shadow-green-900/10 h-[400px] flex flex-col">
      ...
      <div className="p-4 overflow-y-auto flex-1 space-y-2 scrollbar-thin scrollbar-thumb-gray-800 scrollbar-track-transparent">
